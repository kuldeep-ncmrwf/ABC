...
ifort -o ets-4_ku0.5_pctl.exe NE_CT.f
...
mkdir -p NE_scores${iyr}/scores$ip
...
#ls -lrt scrs.txt
cp scrs.txt NE_scores${iyr}/scores$ip/'ukmo_d'$iday'scr_0.5_ku.txt'
cp rfo.txt NE_scores${iyr}/scores$ip/Obs_imdrf_0.5.txt
cp rfm.txt NE_scores${iyr}/scores$ip/'ukmo_d'$iday'rf_0.5.txt'
rm -f scrs.txt
done
done
done
